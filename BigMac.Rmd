...
for (country in selected_countries) {
  ...
  price_data <- wide_bigmac_yearly %>%
    filter(year == country_dfs[[country]]$time) %>%
    pull(country)
    
  # Add the price data as a new column named "Price"
  country_dfs[[country]] <- country_dfs[[country]] %>%
    mutate(Price = price_data)
}
```

And let's interpolate a second time the missing data
```{r}
