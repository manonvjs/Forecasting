...
# Loop over each country and create a dataframe
for (country in selected_countries) {
  # Filter data for the current country
  country_data <- filtered_gdpgrowth %>% 
    filter(ref_area.label == country)
  
  # Create a new dataframe with yearly data
  country_dfs[[country]] <- country_data %>%
    group_by(ref_area.label, time) %>%
    summarise(avg_gdp_growth = mean(obs_value, na.rm = TRUE))
}

# Fetching forecast data from wide_bigmac_yearly
for (country in selected_countries) {
  # Add forecast data to each country's dataframe
  country_dfs[[country]]$Price <- wide_bigmac_yearly %>%
    filter(year == country_dfs[[country]]$time) %>%
    pull(!!rlang::sym(country)) # use sym() from rlang to convert the string to a symbol
}

# Apply interpolation to handle missing data
for (country in selected_countries) {
  # Interpolate missing values for each column in the dataframe
  for (column in c("avg_gdp_growth", "Price")) {
    # If all values are NA, replace them with 0
    if (all(is.na(country_dfs[[country]][[column]]))) {
      country_dfs[[country]][[column]] <- 0
    } else {
      # Interpolate missing values
      country_dfs[[country]][[column]] <- zoo::na.approx(country_dfs[[country]][[column]], rule = 2)
      # Fill remaining NA values (if any) with the closest non-NA value
      country_dfs[[country]][[column]] <- zoo::na.fill(country_dfs[[country]][[column]], fill = list("extend", "extend"))
    }
  }
}

# Loop over each country to fit the model and make predictions
for (country in selected_countries) {
  # Fit multilinear regression model
  model <- lm(Price ~ avg_gdp_growth, data = country_dfs[[country]])

  # Make predictions
  country_dfs[[country]]$predicted_price <- predict(model, newdata = country_dfs[[country]])

  # Plot actual vs predicted
  p <- ggplot(country_dfs[[country]], aes(x = time)) +
    geom_line(aes(y = Price), color = "blue") +
    geom_line(aes(y = predicted_price), color = "red") +
    labs(title = paste("Actual vs Predicted Prices for", country), x = "Year", y = "Price", 
         color = "Legend") +
...
```{r}
fit_best_model <- function(df) {
  ts_data <- ts(df$Price, start = min(df$time), frequency = 1)

  # Fit different models
  ses_model <- tryCatch(ses(ts_data)$model, error = function(e) NULL)
  holt_model <- tryCatch(holt(ts_data)$model, error = function(e) NULL)
  ets_model <- tryCatch(ets(ts_data)$model, error = function(e) NULL)
  arima_model <- tryCatch(auto.arima(ts_data), error = function(e) NULL)

  # Calculate AIC for each model
  models <- list(ses_model, holt_model, ets_model, arima_model)
  aic_values <- sapply(models, function(x) if (!is.null(x)) AIC(x) else Inf)
...
# Initialize a list to hold best models for each country
best_models <- list()

# Fit the best model for each country
for (country in selected_countries) {
  best_models[[country]] <- fit_best_model(country_dfs[[country]])
}


# Forecast using the best models
forecasts <- lapply(best_models, forecast, h = 5)

# Plot the forecasts
for (i in seq_along(selected_countries)) {
  plot(forecasts[[i]], main = paste("Forecast for", selected_countries[i]), xlab = "Year", ylab = "Price")
}
