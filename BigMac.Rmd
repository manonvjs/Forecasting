...
holts_aic <- lapply(holt_models, calc_AIC)
ses_aic <- lapply(ses_models, calc_AIC)
...
results <- data.frame(
  ...
  HOLTS_AIC = sapply(holts_aic, `[[`, "AIC"),
  SES_AIC = sapply(ses_aic, `[[`, "AIC")
)

# Find the model with minimum AIC and BIC for each country
results$Min_AIC_Model <- apply(results[,c("ARIMA_AIC", "ETS_AIC", "HOTLS_AIC", "SES_AIC")], 1, which.min)
...
best_AIC_model <- which.min(c(ARIMA_AIC = mean(sapply(arima_aic, `[[`, "AIC")),
                              ...
                              HOLTS_AIC = mean(sapply(holts_aic, `[[`, "AIC")),
                              SES_AIC = mean(sapply(ses_aic, `[[`, "AIC"))))
...
```
```{r}
selected_countries_without_venezuela <- selected_countries[selected_countries != 'Venezuela']
filtered_inflation <- inf_filtered %>% filter(Country %in% selected_countries_without_venezuela) 
ggplot(data = filtered_inflation, aes(x = Year, y = Inflation_Rate, color = Country)) +
  geom_line() +
  ggtitle("Evolution of Inflation Growth over time by country") +
  xlab("Year") +
...
```
```{r}
arima_models_with_covariates <- list()
...
for (country in names(country_dfs)) {
  ...
  # Check for NA values and handle them appropriately
  if (any(is.na(df))) {
    df[is.na(df)] <- 0  # fill with 0 or use other imputation method
  }
  
  # Extract time series variable
  ts <- ts(df$your_time_series_variable, start = c(min(df$Year), 1), frequency = 1)

  # Create matrix of covariates, excluding the time series variable
  covariates <- as.matrix(df[ , !(names(df) %in% "your_time_series_variable")])
  
  # Fit ARIMA model with covariates
  model <- auto.arima(ts, xreg = covariates)
  
  # Save the model
  arima_models_with_covariates[[country]] <- model
}
