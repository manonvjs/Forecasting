...
For Austria, the model with additive error, no trend and additive seasonality seem better than the other model. SARIMA is also a pretty solid model with a error difference of 2.2. 
...
Same as in Germany, we will choose the model computed by the test / training sets instead of cross validation. Indeed, we got a `SARIMA` with an AIC of `790` instead of the `1271` from the `ETS(A,N,A)` we got in the cross-validation.
...
# Print the forecast
print(forecast_sarima)
print(sarima_model_italy)
```

## Austria

As you might have guessed, the model we got from the test/train split was the best, and we chose the `SARIMA` with an AIC of `3957` instead of the `ETS(A,N,A)` we got from the cross-validation (we got an AIC of `4708` here).

```{r}
# First fit the SARIMA model to the data
sarima_model_austria <- auto.arima(ts_austria, seasonal = TRUE)
...
```{r}
# First fit the SARIMA model to the data
sarima_model_france <- auto.arima(ts_france, seasonal = TRUE)
...
# Print the forecast
print(forecast_sarima)
print(sarima_model_france)
```

## Liechtenstein

For Liechtenstein, we got the same output here and we chose the `SARIMA` model from the test/train split with an AIC of `2327`. The model we got from the 3-fold cross-validation was `ARIMA(0,1,2)` with an AIC of `2770`.

```{r}
# First fit the SARIMA model to the data
sarima_model_liechenstein <- auto.arima(ts_liechtenstein, seasonal = TRUE)
...
# Print the forecast
print(forecast_sarima)
print(sarima_model_lichenstein)
```
...
```{r}
# First fit the SARIMA model to the data
sarima_model_data <- auto.arima(ts_data, seasonal = TRUE)
