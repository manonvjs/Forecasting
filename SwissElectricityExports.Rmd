...
```
To better observe seasonality, we did seasonal subseries plots. You can find the visualisation in the appendix. 
...
In this next chuck of code, we will calculate the accuracy for each model. In order to that, we split the data in two parts. First a training test composed of 80% of the data (from January 2000 to December 2019) and a testing test composed of 20% of the data (from january 2020 to )
...
```
For Austria, we can say the the SAMIRA model is the best
...
```
Samira is the best model for germany accoring to AIC and BIC
...
```
Samira is the best model for Italy accoring to AIC and BIC
...
```
Samira is the best model for Austria accoring to AIC and BIC
...
```
Samira is the best model for France accoring to AIC and Arima is the best according to BIC.
...
```
Samira is the best model for Liechtenstein accoring to AIC and BIC
...
```
Samira is the best model for Italy accoring to AIC and BIC for our overall data. 
...
###Cross validation 
In the next chuck we will perform our cross validation in order to compare the MAE of each models.
...
compare_models_cv <- function(ts_data) {
  ...
  cv_errors_ets_zzz <- tsCV(ts_data, forecastfunction = forecast, h = 1, model = ets_zzz, use.initial.values = FALSE)
  cv_errors_ets_ana <- tsCV(ts_data, forecastfunction = forecast, h = 1, model = ets_ana, use.initial.values = FALSE)
  cv_errors_ets_aaa <- tsCV(ts_data, forecastfunction = forecast, h = 1, model = ets_aaa, use.initial.values = FALSE)
  cv_errors_arima <- tsCV(ts_data, forecastfunction = forecast, h = 1, model = arima_model, use.initial.values = FALSE)
  cv_errors_sarima <- tsCV(ts_data, forecastfunction = forecast, h = 1, model = sarima_model, use.initial.values = FALSE)
  
  if (exists("ets_mnm")) {
    cv_errors_ets_mnm <- tsCV(ts_data, forecastfunction = forecast, h = 1, model = ets_mnm, use.initial.values = FALSE)
  }
...
```
The cross-validation chooses an ETS with Multiplicative error, Additive Trend and Multiplicative seasonality. We can observe that the SARIMA model is not far behind with error difference of 1.2
...
```
For Italy, all the MAE are pretty much the same which can indicate that the models are all good. Our code chooses the one with Additive error, trend and seasonality
...
```
For Austria, the model with additive error, no trend and additive seasonality seem better than the other model. Samira is also a pretty solid model with a error difference of 2.2. 
...
```
For France, the samira model outperforms the other models. 
#Il faudrait traduire ce que ARIMA(2,1,1)(0,0,2) veut dire 
...
```
For Lichtenstein, Sarima/Arima are both the model that are better. Again here the difference of error between the models is pretty low. 
...
```
For the overall data, the MAE is lower for the samira model which is an arima model with seasonality.
...
```{r}
# Generate the 12-month forecast using the best model
forecast_result <- forecast(best_model_germany, h = 12)

# Print the best model
cat("Best model:\n")
print(best_model_germany)

# Print the 12-month forecast with upper and lower intervals
cat("12-month forecast with intervals:\n")
