...
```
For France, the ETS with multiplicative Error and Seasonality seem the best close to the SARIMA model. 
...
```
For Lichtenstein, ARIMA(0,1,2) and SARIMA looks the best according to the Cross-Validation
...
```
For the overall data, the MAE is lower for the SARIMA model. For the non-seasonal part, the AR is equal to 1, the order of differencing to 0 and the MA to 1. For the seaonal part, the AR is equal to 2, the seasonal autoregressive term to 0 and the seasonal MA is equal to 0.  
...
```{r}
# First fit the SARIMA model to the German data
sarima_model_germany <- auto.arima(ts_germany, seasonal = TRUE)
